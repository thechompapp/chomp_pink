...
const SearchBar = ({ 
  ...
  const performSearch = useCallback(async (searchTerm) => {
    console.log(`[SearchBar performSearch] Starting search for: "${searchTerm}"`);
    if (!searchTerm || searchTerm.trim().length < 2) {
      console.log(`[SearchBar performSearch] Search term too short or empty`);
    ...
    setError(null);
    try {
      // Always search 'all' types for the dropdown to show comprehensive results
      const searchType = 'all';
      console.log(`[SearchBar performSearch] Calling searchService.search with:`, { q: searchTerm.trim(), type: searchType });
      const response = await searchService.search({ q: searchTerm.trim(), type: searchType });
      console.log(`[SearchBar performSearch] Search response received:`, response);
      console.log(`[SearchBar performSearch] Response type:`, typeof response);
      console.log(`[SearchBar performSearch] Response keys:`, response ? Object.keys(response) : 'no response');
      ...
      console.log(`[SearchBar performSearch] Results set:`, {
        dishes: Array.isArray(response?.dishes) ? response.dishes.length : 0,
        restaurants: Array.isArray(response?.restaurants) ? response.restaurants.length : 0,
        lists: Array.isArray(response?.lists) ? response.lists.length : 0,
      });
    } catch (err) {
      console.error(`[SearchBar performSearch] Search error:`, err);
      setError('Search failed. Please try again.');
      console.error("Search error:", err);
      setResults({ dishes: [], restaurants: [], lists: [] });
    } finally {
      setIsSearching(false);
      console.log(`[SearchBar performSearch] Search completed`);
    }
  }, []); // Remove contentType dependency and use 'all' directly
  ...
  const renderResultItem = (item, type, Icon) => {
    // Map search result types to correct URL patterns
    const getDetailPath = (type, item) => {
      switch (type) {
        case 'dish':
          return `/dish/${item.id}`;
        case 'restaurant':
      ...
      <form onSubmit={handleSearchSubmit} className="flex items-center w-full">
        <div className="relative flex-grow">
          ...
          <input
            ...
            className="block w-full pl-9 pr-10 py-2 border border-gray-300 rounded-md leading-5 bg-white placeholder-gray-500 focus:outline-none focus:placeholder-gray-400 focus:ring-1 focus:ring-gray-400 focus:border-gray-400 sm:text-sm"
