...
// No shallow needed when selecting primitives individually
...
function AddToListModal({ isOpen, onClose, item }) {
    ...
    // FIX: Select state slices individually to ensure stable references for useSyncExternalStore
    const userLists = useUserListStore(state => state.userLists);
    const fetchUserLists = useUserListStore(state => state.fetchUserLists); // Function reference is stable
    const isLoadingLists = useUserListStore(state => state.isLoading);
    const listsError = useUserListStore(state => state.error);
    const userId = useAuthStore((state) => state.user?.id); // Keep userId check
    ...
    useEffect(() => {
        if (isOpen) {
            // Read current state directly inside effect for condition check
            ...
            if (shouldFetch && userId) {
                console.log("[AddToListModal] Conditions met inside effect (individual selection), fetching lists...");
                fetchUserLists();
            } else {
                 console.log(`[AddToListModal] Skipping fetch inside effect (individual selection). ShouldFetch: ${shouldFetch}, UserID: ${userId}`);
            }
        } else {
    ...
    }, [isOpen, fetchUserLists, userId]); // Dependencies: isOpen triggers run, fetchUserLists is stable, userId guards fetch


    const addItemMutation = useMutation({
        mutationFn: ({ listId, itemId, itemType }) => addItemToList(listId, { item_id: itemId, item_type: itemType }),
    ...
    // Use the subscribed state variables for rendering
            ...
            <form onSubmit={handleSubmit}>
                ...
                <div className="flex justify-end space-x-3 mt-5">
                    ...
                    <Button
                        type="submit"
                        variant="primary"
                        // Disable if mutating, loading lists, lists had error, or no selection
