...
const FloatingQuickAddComponent = () => {
  ...
  const tagSuggestions = useMemo(() => {
    if (!formState.tagInput || formState.tagInput.trim().length === 0 || !Array.isArray(cuisines)) return [];
    const inputLower = formState.tagInput.trim().toLowerCase();
    return cuisines.filter(cuisine => cuisine?.name?.toLowerCase().includes(inputLower) && !formState.selectedTags.includes(cuisine.name)).map(cuisine => cuisine.name).slice(0, 5);
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      ...
      if (mainContainer && !mainContainer.contains(event.target) && !(fabButton && fabButton.contains(event.target))) {
        ...
        if (showTagSuggestionsUI && tagInputRef.current && !tagInputRef.current.contains(event.target) && !(tagSuggestionsList && tagSuggestionsList.contains(event.target))) {
          setShowTagSuggestionsUI(false);
        }
        const restaurantSuggestionsList = mainContainer.querySelector('#fq-restaurant-suggestions');
        if (showRestaurantSuggestionsUI && restaurantInputRef.current && !restaurantInputRef.current.contains(event.target) && !(restaurantSuggestionsList && restaurantSuggestionsList.contains(event.target))) {
          setShowRestaurantSuggestionsUI(false);
        }
      }
    };
  ...
  const handleSelectRestaurantSuggestion = useCallback(async (suggestion) => {
    ...
    try {
      ...
      if (details && typeof details === 'object') {
        ...
        throw new Error("Received invalid details object from service.");
      }
    } catch (error) {
      console.error("[FloatingQuickAdd] Error fetching/setting place details:", error);
      setSubmitError("Could not fetch details.");
  ...
  const handleRestaurantInputChange = (event) => {
    handleChange(event);
    if (formState.placeDetails) setFormState(prev => ({ ...prev, placeDetails: null }));
  };

  const handleRestaurantInputFocus = () => {
    if (formState.restaurantSuggestions.length > 0) setShowRestaurantSuggestionsUI(true);
    if (formData.restaurantInput && formData.restaurantInput.trim().length >= 2) fetchPlaceSuggestions(formData.restaurantInput);
  };
  ...
  return (
    <>
      ...
      >
        <span className={isMenuOpen ? 'transform rotate-45 transition-transform duration-200 ease-in-out' : 'transform rotate-0 transition-transform duration-200 ease-in-out'}>
          <Plus size={28} />
        </span>
      </button>
      ...
      >
        {!formState.formType ? (
          <div className="bg-white border border-gray-200 rounded-lg shadow-xl p-4 space-y-3">
            ...
            <Button onClick={handleOpenDishForm} variant="tertiary" className="w-full flex items-center justify-start space-x-2 text-gray-700 hover:bg-gray-100">
              <Utensils size={16} /> <span>Add New Dish...</span>
            </Button>
            <Button onClick={handleOpenRestaurantForm} variant="tertiary" className="w-full flex items-center justify-start space-x-2 text-gray-700 hover:bg-gray-100">
              <Store size={16} /> <span>Add New Restaurant...</span>
            </Button>
            <Button onClick={handleCreateNewList} variant="tertiary" className="w-full flex items-center justify-start space-x-2 text-gray-700 hover:bg-gray-100">
          ...
          <form onSubmit={handleFormSubmit} className="bg-white border border-gray-200 rounded-lg shadow-xl p-4">
            <div className="flex items-center justify-between mb-4">
              ...
              <Button onClick={() => { setFormState((prev) => ({ ...prev, formType: null })); resetAllState(); }} variant="tertiary" size="sm" className="p-1 text-gray-400 hover:text-gray-600" aria-label="Back to quick add menu">
                <X size={18} />
              </Button>
            </div>
            <div className="space-y-3 text-sm">
                ...
                <div ref={restaurantInputRef} className="relative">
                  ...
                  <div className="relative">
                    ...
                    {isFetchingPlaceSuggestions && (
                      <Loader2 size={16} className="absolute right-2 top-1/2 transform -translate-y-1/2 animate-spin text-gray-400" />
                    )}
                  </div>
                          ...
                          suggestion?.placeId && (
                            <li key={`rest-sugg-${suggestion.placeId || index}`}>
                              <button
                                ...
                                onMouseDown={(e) => { e.preventDefault(); e.stopPropagation(); handleSelectRestaurantSuggestion(suggestion); }}
                              >
                                {suggestion?.name || 'Invalid suggestion'}
                              </button>
                            </li>
                          )
                        ))}
              ...
              <div ref={tagInputRef} className="relative">
                ...
                <input
                  ...
                  onChange={(e) => { setFormState((prev) => ({ ...prev, tagInput: e.target.value })); setShowTagSuggestionsUI(true); setSubmitError(null); }}
                  onKeyDown={handleTagInputKeyDown}
                  onFocus={() => setShowTagSuggestionsUI(true)}
                  disabled={isSubmitting}
                  className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-1 focus:ring-[#D1B399] focus:border-[#D1B399]"
                  placeholder="Type to find matching tags"
                  autoComplete="off"
                />
                {showTagSuggestionsUI && Array.isArray(tagSuggestions) && tagSuggestions.length > 0 && (
              ...
              {submitError && (
                <p role="alert" className="text-sm text-red-600 bg-red-50 border border-red-200 rounded p-2 mt-2 text-center">{submitError}</p>
              )}
              <Button
                type="submit"
                variant="primary"
