...
const QuickCreateForm = ({ onClose }) => {
  ...
  // Mock data and all other functions remain the same
  ...
  const getSuggestions = () => {
    return creationType === 'restaurant' 
      ? suggestedRestaurants.filter(item => item.name.toLowerCase().includes(nameInput.toLowerCase()))
      : suggestedDishes.filter(item => item.name.toLowerCase().includes(nameInput.toLowerCase()));
  };
    ...
    <div className="max-w-md mx-auto bg-white rounded-xl shadow-md overflow-hidden">
      ...
      <div className="p-4 border-b">
        <div className="flex rounded-lg overflow-hidden">
          <button 
            className={`flex-1 py-2 px-4 text-center ${creationType === 'restaurant' 
          ...
          <button 
            className={`flex-1 py-2 px-4 text-center ${creationType === 'dish' 
              ? 'bg-pink-500 text-white' 
              : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
