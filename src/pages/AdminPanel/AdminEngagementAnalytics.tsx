// src/pages/AdminPanel/AdminEngagementAnalytics.tsx
...
import { Activity, BarChart2, Filter, MapPin, Tag, Hash } from 'lucide-react';
...
// --- Fetcher Functions ---
const fetchEngagementDetails = async (): Promise<EngagementDetails> => {
  console.log(`[AdminEngagementAnalytics] Fetching engagement details...`);
  const endpoint = `/api/analytics/engagements`;
  try {
    const response = await apiClient<{ data: EngagementDetails }>(endpoint, `Admin Fetch Engagement Details`);
...
const AdminEngagementAnalytics: React.FC = () => {
  const [drilldownView, setDrilldownView] = useState<string>('platform');
  const [selectedFilter, setSelectedFilter] = useState<string | null>(null);
  const [chartItemType, setChartItemType] = useState<string>('restaurant');

  const engagementDetailsQuery = useQuery<EngagementDetails, Error>({
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-center gap-2 mt-2">
        {(['restaurant', 'dish', 'list'] as const).map((type) => (
          <Button
            key={type}
      ...
      <div className="p-4 bg-gray-50 border border-gray-200 rounded-lg">
        ...
        <div className="flex flex-wrap gap-2">
          <Button
            variant={drilldownView === 'platform' ? 'primary' : 'tertiary'}
            size="sm"
            onClick={() => {
              setDrilldownView('platform');
          ...
          <Button
            variant={drilldownView === 'dish' ? 'primary' : 'tertiary'}
            size="sm"
            onClick={() => {
              setDrilldownView('dish');
              setSelectedFilter(null);
            }}
          >
            By Dish (Aggregate)
          ...
          <Button
            variant={drilldownView === 'restaurant' ? 'primary' : 'tertiary'}
            size="sm"
            onClick={() => {
              setDrilldownView('restaurant');
              setSelectedFilter(null);
            }}
          >
            By Restaurant (Aggregate)
          </Button>
          <Button
            variant={drilldownView === 'category' ? 'primary' : 'tertiary'}
            size="sm"
            onClick={() => {
          ...
          <Button
            variant={drilldownView === 'neighborhood' ? 'primary' : 'tertiary'}
            size="sm"
            onClick={() => {
              setDrilldownView('neighborhood');
              setSelectedFilter(null);
            }}
          >
            By Neighborhood
          ...
          <Button
            variant={drilldownView === 'cuisine' ? 'primary' : 'tertiary'}
            size="sm"
            onClick={() => {
              setDrilldownView('cuisine');
              setSelectedFilter(null);
            }}
          >
            By Cuisine
        ...
        {drilldownView !== 'platform' && drilldownView !== 'dish' && drilldownView !== 'restaurant' && (
          <div className="mt-4">
            <label htmlFor="drilldown-filter" className="text-sm font-medium text-gray-700 mr-2">
              Filter by {drilldownView}:
            </label>
            <input
              id="drilldown-filter"
              type="text"
