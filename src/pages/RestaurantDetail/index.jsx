...
import * as RadixTabs from '@radix-ui/react-tabs'; // Import Radix UI Tabs
import {
...
import ListCard from '@/pages/Lists/ListCard'; // Reusing for "Featured On"
import RestaurantCard from '@/components/UI/RestaurantCard'; // Reusing for "Similar Places"

// Fetcher function - Needs to align with updated backend response structure eventually
const fetchRestaurantDetails = async (restaurantId) => {
    // Corrected: Use standard property assignment for the error object
    if (!restaurantId) {
        const error = new Error('Restaurant ID is required.');
        error.status = 400; // Assign status property directly
        throw error;
    }
    try {
        // Expecting RestaurantDetail type from the service now
        const response = await restaurantService.getRestaurantDetails(restaurantId);
        if (!response || typeof response.id === 'undefined') {
             const error = new Error('Restaurant not found.');
             error.status = 404; // Assign status property directly
            throw error;
        }
        // Add basic default values for potentially missing new fields for now
        // This ensures the component doesn't break if backend isn't updated yet
        return {
            ...response,
            rating: response.rating ?? null,
            primary_category: response.primary_category ?? (response.tags?.[0] || null), // Guess category from first tag if needed
            ...
            // Ensure "The Take" fields are defaulted if missing from backend
        ...
        // Corrected: Use standard property assignment if adding status here too
        const fetchError = new Error( error.message || 'Failed to load restaurant details');
        fetchError.status = error.status || 500; // Assign status property directly
        throw fetchError;
    }
};
...
const RestaurantDetail = () => {
    ...
    const [activeTab, setActiveTab] = useState('getting-there'); // Default tab
    ...
    const queryResult = useQuery({
        ...
        enabled: !!id,
        staleTime: 5 * 60 * 1000,
        retry: (failureCount, error /* Removed type assertion */) => error?.status !== 404 && failureCount < 1, // Use optional chaining
    ...
    // Quick Add Handler for the main "Add to List" button
    const handleAddToList = useCallback((restaurant) => {
         if (!isAuthenticated) {
             navigate('/login', { state: { from: location } });
             return;
         }
         if (!restaurant || typeof restaurant.id === 'undefined') return; // Add guard
         openQuickAdd({
             type: 'restaurant',
             id: restaurant.id,
     ...
     // Handler for adding dishes from the "What to Order" tab
     const handleDishQuickAdd = useCallback((dish, restaurantName) => {
         if (!isAuthenticated) {
             navigate('/login', { state: { from: location } });
             return;
         }
         if (!dish || typeof dish.id === 'undefined') return; // Add guard
         openQuickAdd({
             type: 'dish',
             id: dish.id,
             name: dish.name,
    ...
    const formatWebsiteUrl = (url) => {
        ...
        return `http://${url}`; // Assume http if protocol missing
    };
    ...
    return (
        <div className="p-3 md:p-5 max-w-4xl mx-auto text-gray-900">
             ...
             >
                 ...
                 <span className="transition-colors group-hover:text-[#A78B71]">Back to search</span>
             </button>
                    ...
                    <div className="space-y-6">
                            ...
                            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                                ...
                                <div className="flex-shrink-0 flex items-center gap-2 w-full sm:w-auto">
                                    {restaurant.website && formatWebsiteUrl(restaurant.website) && (
                                        <a
                                            href={formatWebsiteUrl(restaurant.website) ?? '#'}
                                            target="_blank"
                                            rel="noopener noreferrer"
                                            className="flex-1 sm:flex-none"
                                        >
                                            <Button as="span" variant="primary" size="sm" className="w-full !bg-[#FF69B4] hover:!bg-[#f550a3]">
                                                <ExternalLink size={14} className="mr-1" /> Visit
                                            </Button>
                                        </a>
                                    )}
                                    <Button
                                         variant="secondary" size="sm" onClick={() => handleAddToList(restaurant)}
                                         className="flex-1 sm:flex-none !border-[#FF69B4] !text-[#FF69B4] hover:!bg-[#FF69B4]/10"
                                     >
                                        <PlusCircle size={14} className="mr-1" /> Add to List
                                     </Button>
                                </div>
                            </div>
                        </div>

                        {/* --- "The Take" Section (Added Back) --- */}
                        ...
                        <RadixTabs.Root value={activeTab} onValueChange={setActiveTab} className="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden">
                            <RadixTabs.List className="flex border-b border-gray-200 bg-gray-50">
                                <RadixTabs.Trigger value="getting-there" className={`flex-1 py-2 px-4 text-sm font-medium focus:outline-none focus:ring-1 focus:ring-inset focus:ring-[#D1B399] transition-colors ${activeTab === 'getting-there' ? 'text-[#A78B71] border-b-2 border-[#A78B71] bg-white' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-100'}`}>
                                    Getting There
                                </RadixTabs.Trigger>
                                <RadixTabs.Trigger value="what-to-order" className={`flex-1 py-2 px-4 text-sm font-medium focus:outline-none focus:ring-1 focus:ring-inset focus:ring-[#D1B399] transition-colors ${activeTab === 'what-to-order' ? 'text-[#A78B71] border-b-2 border-[#A78B71] bg-white' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-100'}`}>
                                    What to Order
                                </RadixTabs.Trigger>
                            </RadixTabs.List>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div>
                                        <div className="aspect-video bg-gray-200 rounded flex items-center justify-center text-gray-500">
                                            ...
                                            {/* Consider adding a link using google_place_id or lat/lng here */}
                                             {restaurant.google_place_id && (
                                                 <a href={`https://www.google.com/maps/search/?api=1&query=a&query_place_id=${restaurant.google_place_id}`} target="_blank" rel="noopener noreferrer" className="text-xs text-blue-600 hover:underline ml-2">(View on Google Maps)</a>
                                             )}
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-3">
                                     {restaurant.featured_on_lists.map((list) => (
                                            <div key={`featured-${list.id}`} className="bg-white border border-gray-200 p-3 rounded-md flex justify-between items-center text-sm">
                                                 ...
                                                 {isAuthenticated && ( // Only show add button if logged in
                                                      <Button
                                                         size="xs"
                                                         variant="tertiary"
                                                         title={`Add ${restaurant.name} to your list`}
                                                         onClick={() => handleAddToList(restaurant)} // Use main restaurant add handler
                                                         className="!p-1.5"
                                                     >
                                                          <PlusCircle size={16}/>
                                                      </Button>
