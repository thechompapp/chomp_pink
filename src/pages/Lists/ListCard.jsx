...
const ListCard = React.memo(
  ({ id, name, itemCount, savedCount, city, tags, isFollowing = false, createdByUser = false, creatorHandle = "@user" }) => {
    return (
      <div className="w-72 bg-white border border-gray-200 rounded-lg p-4 shadow-sm hover:shadow-md relative">
        {!createdByUser && (
          <div className="absolute top-3 right-3">
            <FollowButton listId={id} isFollowing={isFollowing} />
          </div>
        )}
        ...
        <div className="flex flex-col h-full">
          <div className="flex-1">
            <Link to={`/lists/${id}`} className="block">
              <h3 className="text-lg font-bold text-gray-900 mb-2 truncate hover:text-[#D1B399]">{name || "Unnamed List"}</h3>
            </Link>
