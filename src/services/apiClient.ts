...
export interface ApiResponse<T = any> {
  data: T | null; // Always present, null if no data
  ...
  try {
    ...
    try {
      ...
      const standardizedResponse: ApiResponse<T> = {
        data: jsonData.data ?? jsonData ?? null, // Fallback to full response if no 'data' key
        success: jsonData.success !== false, // Default to true unless explicitly false
