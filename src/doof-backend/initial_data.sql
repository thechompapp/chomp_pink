-- src/doof-backend/initial_data.sql (Corrected for Lists table schema)
...
-- Populate Lists table (Corrected: REMOVED is_following column from INSERT)
INSERT INTO Lists (id, name, description, saved_count, city_name, tags, is_public, created_by_user, creator_handle) VALUES
(1, 'NYC Pizza Tour', 'Best slices in the city', 245, 'New York', ARRAY['pizza', 'italian', 'nyc', 'slice'], true, false, '@pizzalover'),
(2, 'Iconic NYC Eats', 'Must-try classic spots', 187, 'New York', ARRAY['classic', 'deli', 'sandwiches', 'cheap eats', 'must try'], true, true, '@nycfoodie')
...
-- Populate ListItems table (Should work now that Lists insert is fixed)
...
ON CONFLICT (list_id, item_type, item_id) DO NOTHING;
-- Reset sequence for ListItems safely
