...
const USERS = [
  { email: 'test@example.com', password: 'password123', username: 'testuser', role: 'user' },
  { email: 'admin@example.com', password: 'doof123', username: 'admin', role: 'admin' }
];
...
async function ensureUser(user) {
  try {
    ...
    if (res.data && res.data.id) {
      console.log(`User ${user.email} already exists (id: ${res.data.id})`);
      return res.data;
    }
  } catch (err) {
