...
// Helper function to register a test user if it doesn't exist
const ensureTestUser = async (userData) => {
  try {
    ...
    if (!registerResult.success) {
      ...
      throw new Error(`Failed to register test user: ${userData.email}`);
    }
...
describe('Authentication Endpoints', () => {
  ...
  beforeAll(async () => {
    ...
    try {
      await ensureTestUser(testUsers.admin);
      await ensureTestUser(testUsers.regular);
    } catch (error) {
      console.warn('Failed to ensure test users:', error.message);
