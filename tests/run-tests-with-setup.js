 ...
 * It checks that the backend server is running and then executes the tests.
 */
...
import { ensureBackendRunning } from './setup/ensure-backend.js';
...
async function main() {
  ...
  if (!backendRunning) {
    console.error(chalk.red('Failed to ensure backend server is running. Tests may fail due to connection issues.'));
