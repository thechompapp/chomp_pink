...
import { 
  ...
  createRestaurant,
  cleanupTestData
} from '../../../src/__tests__/setup/robust-api-client.js';
import { generateTestUser } from '../../../src/__tests__/setup/test-users.js';
import { initializeDatabase } from '../../../src/__tests__/setup/db-init.js';
...
describe('Bulk Add Semicolon Format E2E Tests', function() {
  ...
  it('should process semicolon-delimited input format', async function() {
    ...
    try {
      ...
      expect(testRestaurants).toHaveLength(sampleInput.length);
      ...
      // Verify list details
      expect(listDetailsResponse).toHaveProperty('success', true);
      expect(Array.isArray(listDetailsResponse.data.items)).toBe(true);
      ...
      for (const restaurant of testRestaurants) {
        ...
        expect(foundItem.notes).toContain('Added via semicolon format test');
      }
